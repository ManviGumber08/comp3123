...
**6. Explain the Purpose of `express.Router()` in the Code Above.**
 express.Router() is a built-in class in Express.js that provides a way to create modular, mountable route handlers. It allows you to group routes and middleware together, which helps in organizing code better. You can create separate route files for different parts of your application (e.g., user routes, admin routes), and then import them into your main application file. This modular approach keeps your code clean and maintainable.
...
we  can implement error handling by creating a middleware function that takes four parameters (err, req, res, next). 
app.use((err, req, res, next) => {
    console.error(err);
...
The line app.listen(process.env.PORT || 8081) allows you to set the port dynamically based on the environment. If process.env.PORT is defined (which is common in production environments like Heroku), it uses that value. Otherwise, it defaults to 8081 during local development. This is useful because it enables your application to run seamlessly in different environments without changing the code.
---
